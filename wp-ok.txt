...
function readlink_absolute_path($path){
    ...
    if (($is_windows && substr($target,2,1) != ':') || (!$is_windows && substr($target,0,1) != DIRECTORY_SEPARATOR)){
        $target = substr($path,0,strrpos($path,DIRECTORY_SEPARATOR)+1).$target;
    }
    return $target;
}
